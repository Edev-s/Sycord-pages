...
import AIWebsiteBuilder, {
  type GeneratedPage,
} from "@/components/ai-website-builder";
import {
  Trash2,
  Plus,
  ExternalLink,
...
const paymentOptions = [
  {
    id: "stripe",
    name: "Stripe",
    description: "Credit cards and digital wallets",
  },
    ...
    <div className="flex flex-col h-full p-4">
      <div className="flex items-center gap-3 mb-8 px-2 text-foreground">
        ...
        <span className="font-bold text-lg truncate">
          {project?.businessName || "Site Settings"}
        </span>
      </div>
...
export default function SiteSettingsPage() {
  ...
  const fetchLogs = async (repoIdOverride?: string) => {
    ...
    try {
      const res = await fetch(
        `https://micro1.sycord.com/api/logs?project_id=${targetId}&limit=50`,
      );
      if (res.ok) {
        ...
        if (data.success && Array.isArray(data.logs)) {
          ...
          const urlMatch = combinedLogs.match(
            /Take a peek over at[\s\S]*?(https:\/\/[a-zA-Z0-9.-]+\.pages\.dev)/,
          );
          ...
          const successFound =
            combined.includes("take a peek over at") ||
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (
        ...
        console.warn(
          "Image upload not fully implemented in this example. Placeholder logic used.",
        );
      ...
      if (projectUpdateNeeded) {
        ...
        if (!projectResponse.ok) {
          ...
          throw new Error(
            errorData.message || "Failed to update project details",
          );
        }
      ...
      if (!settingsResponse.ok) {
        throw new Error(
          settingsResponseData.message || "Failed to save settings",
        );
      }
  ...
  const handleDeleteProduct = async (
    ...
    try {
      const response = await fetch(
        `/api/projects/${id}/products?productId=${productId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleDeletePage = async (pageName: string) => {
    ...
    try {
      const response = await fetch(
        `/api/projects/${id}/pages?name=${encodeURIComponent(pageName)}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      setDeployResult({
        ...
        message:
          result.message ||
          `Successfully deployed ${result.filesCount} file(s) to GitHub`,
      });
  ...
  const stats = {
    ...
    storage: {
      used: parseFloat(
        (generatedPages.length * 0.2 + products.length * 0.1).toFixed(1),
      ),
      limit: 50,
      label: "MB STORAGE",
      icon: HardDrive,
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header
          className={cn(
            "border-b border-white/10 bg-background/50 backdrop-blur-sm z-20 shrink-0",
          )}
        >
        ...
        <main
          className={cn(
            ...
            activeTab === "ai"
              ? "p-0 overflow-hidden"
              : "overflow-y-auto overflow-x-hidden p-4 md:p-6 lg:p-8 custom-scrollbar",
          )}
        >
          <div
            className={cn(
              "mx-auto",
              activeTab === "ai"
                ? "h-full w-full max-w-none p-0 pb-0 space-y-0"
                : "max-w-6xl space-y-8 pb-10",
            )}
          >
            <AnimatePresence>
              {isSidebarOpen && (
            ...
            {/* TAB CONTENT: STYLES */}
            {activeTab === "styles" && (
              <div className="max-w-xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 pt-4">
                {/* Preview Area */}
                <div className="aspect-[4/3] w-full bg-card rounded-[24px] relative overflow-hidden border border-white/5 shadow-lg">
                  {previewUrl ? (
                    <iframe
                      src={previewUrl}
                  ...
                  {previewUrl && (
                    <div className="absolute bottom-4 left-0 flex items-center">
                      <div className="bg-emerald-500 h-8 w-8 rotate-45 -ml-4 flex items-center justify-center rounded-sm">
                        <div className="-rotate-45">
                          <Sparkles className="h-4 w-4 text-white" />
                        </div>
                      </div>
                      <div className="bg-emerald-500 text-white text-sm font-semibold px-4 py-2 -ml-2 rounded-r-md">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between px-2">
                    ...
                    {previewUrl ? (
                      <Button
                        variant="ghost"
                        className="h-8 bg-white/5 hover:bg-white/10 text-muted-foreground hover:text-foreground rounded-md transition-colors px-4 text-xs font-medium"
                      ...
                      <Button
                        variant="ghost"
                        className={cn(
                          "h-8 rounded-md transition-colors px-4 text-xs font-medium",
                      ...
                      >
                        {isDeploying ? (
                          ...
                          <>
                            <Sparkles className="h-3.5 w-3.5 mr-2" /> Fix with
                            AI
                          </>
                        ) : (
                          <>
                  ...
                  {deployError && (
                    <div className="px-2 text-sm text-destructive flex items-center justify-between">
                ...
                <div
                  ...
                  className="h-24 w-full bg-card/50 border border-white/5 rounded-[20px] flex items-center justify-center cursor-pointer hover:bg-white/5 transition-colors group"
                >
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="aspect-[4/3] bg-card/50 border border-white/5 rounded-[20px] flex flex-col items-center justify-center text-sm gap-2">
                      ...
                      {productsLoading ? (
                        ...
                        <div className="space-y-4">
                          {products.map((product) => (
                            <div
                              key={product._id}
                              className="group flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 rounded-lg bg-black/20 border border-white/5 hover:border-white/10 transition-colors"
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  ...
                                  <span
                                    className={`text-[10px] uppercase px-1.5 py-0.5 rounded ${product.inStock ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
                                  >
                                    {product.inStock
                                      ? "In Stock"
                                      : "Out of Stock"}
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="flex items-center gap-2 pt-2">
                        <Switch
                          ...
                          onCheckedChange={(c) =>
                            setNewProduct({ ...newProduct, inStock: c })
                          }
                        />
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {paymentOptions.map((option) => (
                    <Card
                      key={option.id}
                      className="bg-card/50 backdrop-blur-sm border-white/10 hover:border-primary/50 transition-all"
                    >
                      <CardHeader>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      Manage AI-generated content (Vite + TypeScript)
                    </p>
                  </div>
                    ...
                    {generatedPages.length > 0 && (
                      <Button
                        ...
                        onClick={async () => {
                          if (
                            !confirm(
                              "Are you sure you want to delete ALL generated pages? This cannot be undone.",
                            )
                          )
                            return;
                          try {
                            const res = await fetch(
                              `/api/projects/${id}/pages?all=true`,
                              { method: "DELETE" },
                            );
                            if (res.ok) {
                              setGeneratedPages([]);
                ...
                {generatedPages.length === 0 ? (
                  <div className="border-2 border-dashed border-white/10 rounded-xl p-12 text-center bg-white/5">
                    ...
                    <Button variant="link" onClick={() => setActiveTab("ai")}>
                      Go to AI Builder
                    </Button>
                  </div>
                ) : (
                          ...
                          {selectedPage && (
                            <div className="flex items-center gap-2">
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  handleDeletePage(selectedPage.name)
                                }
                              >
            ...
            {["orders", "customers", "analytics", "discount"].includes(
              activeTab,
            ) && (
