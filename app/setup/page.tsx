...
def wildcard_router(path):
    ...
    # Serve the runner index page if accessed via the root domain
    if host == 'sycord.site':
        ...
        <html>
          ...
          <body>
            <div class="container">
              ...
              <p>Your Sycord deployment server is running successfully behind Cloudflare on the root domain.</p>
              <p style="font-size: 0.8rem; margin-top: 1rem; color: #555;">Listening for AI deployments on /api/deploy/&lt;project_id&gt;</p>
            </div>
          </body>
        </html>
        """
          ...
          <div className="p-6 border border-green-500/50 bg-green-500/10 rounded-lg flex flex-col items-center space-y-6 animate-in zoom-in duration-500">
             ...
             <div className="w-full max-w-2xl bg-background border border-border rounded-xl p-6 space-y-4">
                ...
                <p className="text-sm text-muted-foreground">
                   To ensure AI-generated websites can be deployed directly to subdomains (<code className="text-primary">project.sycord.site</code>), you must configure your Cloudflare DNS to route traffic to your new tunnel.
                </p>
                <div className="bg-accent/30 p-4 rounded-lg border border-border space-y-4">
                   <div>
                     <p className="text-sm font-medium mb-2">1. Add the Root Record (for the Runner API):</p>
                     <ul className="text-sm space-y-1 font-mono text-zinc-300">
                   ...
                   <div>
                     <p className="text-sm font-medium mb-2">2. Add the Wildcard Record (for generated sites):</p>
                     <ul className="text-sm space-y-1 font-mono text-zinc-300">
                        <li><span className="text-zinc-500 inline-block w-16">Type:</span> CNAME</li>
                        <li><span className="text-zinc-500 inline-block w-16">Name:</span> *</li>
                        <li>
                   ...
                   <p className="text-xs text-muted-foreground mt-4 pt-4 border-t border-border/50">
                     <strong>Note:</strong> By routing deployments to a first-level subdomain, Cloudflare's Free Universal SSL will automatically secure all websites hosted on the platform out-of-the-box. The VPS system will automatically attempt to configure DNS via the Cloudflare API during deployments if API keys are provided.
                   </p>
                </div>
             </div>
