...
async function deployToPages(accountId: string, projectName: string, files: Record<string, string>, apiToken: string) {
    ...
    for (const [path, content] of Object.entries(files)) {
        ...
        // Store buffer for later appending
        fileBuffers.push({
            hash, 
            buffer,
            contentType
    ...
    formData.append("manifest", manifestBuffer, {
        filename: "manifest.json",
        contentType: "application/json",
    });

    // 3. Append all file buffers using their hash as the key
    ...
    // We must pass the headers generated by form-data
    const deployRes = await cloudflareApiCall(
        deployUrl,
        {
            method: "POST",
            body: formData as any, // Cast to any because native fetch expects native BodyInit
