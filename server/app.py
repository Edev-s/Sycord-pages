...
def index():
    ...
    if mongo_uri:
        try:
            ...
            project_rows = []
            for user in users:
                username = html.escape(str(user.get("name", "Unknown User")))
                for project in user.get("projects", []):
                    ...
                    if git_url:
                        git_url = html.escape(str(git_url))
                        pid = html.escape(str(project.get("_id", "")))
                        subdomain = html.escape(str(project.get("subdomain", "")))

                        # Use data attributes instead of inline onclick for safer JS
                        ...
                        <div class="project-row">
                            ...
                            <button data-pid="{pid}" data-sub="{subdomain}" data-url="{git_url}" class="fill-btn">Deploy</button>
                        </div>
                        ''')
        ...
        <script>
            document.querySelectorAll('.fill-btn').forEach(btn => {
                btn.addEventListener('click', () => {
                    document.getElementById('project-id').value = btn.dataset.pid;
                    document.getElementById('subdomain').value = btn.dataset.sub;
                    document.getElementById('github-url').value = btn.dataset.url;
                });
            });
    ...
    """.replace("__PROJECTS_LIST_HTML__", projects_list_html)
